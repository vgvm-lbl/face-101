...
for name, edges in FACE_MESH_SETS.items():
    adj = defaultdict(set)
    for a, b in FACEMESH_TESSELATION:
        adj[a].add(b)
        adj[b].add(a)

    # Find unique triangles (A,B,C where all edges exist)
    triangles = set()
    for a, b in FACEMESH_TESSELATION:
    ...
    tri_strings = [f"[{tri[0]},{tri[1]},{tri[2]}]" for tri in triangles]
    print(f"const {name}_TRIANGLES = [{','.join(tri_strings)}];")
    continue


    # Output JS array
    print("const TRIANGLES = [")
